...
export default function ProjectsCarousel() {
    ...
    return (
        <div className="w-screen flex flex-col items-center justify-center overflow-hidden p-8 gap-8">
            
            {/* 상단: 현재 아이템 + 설명란 */}
            <div className="w-full max-w-7xl flex-1 flex gap-8 border rounded-3xl">
                
                {/* 왼쪽: 현재 프로젝트 카드 */}
                <motion.div
                    key={currentIndex}
                    className="flex-1 rounded-3xl shadow-2xl flex items-center justify-center overflow-hidden relative"
                    style={{ aspectRatio: '2/3' }}  // 2:3 비율 고정!
                ...
                >
                    {/* 블러 배경 레이어 */}
                    <div
                        className="absolute inset-0 scale-130 blur-[40px] saturate-150 brightness-75"
                        style={{
                    ...
                    />
                    <img 
                        className="w-full h-1/2 object-scale-down relative z-10"
                        //   style={{ transform: 'scale(1.19)' }}  // 90%로 축소
                ...
                <motion.div 
                    ...
                    className="flex-1 flex flex-col justify-center bg-blue-900 text-white font-paperlogy"
                    initial={{ opacity: 0, x: 50 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.5, delay: 0.2 }}
                >
                    <h2 className="bg-blue-900 text-5xl font-bold py-4">
                        {currentItem.title}
                    </h2>
